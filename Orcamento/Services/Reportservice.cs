...
/// <summary>
/// Servi�o para gera��o de relat�rios e estat�sticas
/// </summary>
...
var dataInicio = new DateTime(ano, mes, 1);
 ...
 return gastosPorCategoria.OrderByDescending(g => g.ValorTotal).ToList();
    ...
    #region Tend�ncias e Previs�es

    public async Task<PrevisaoGastos> PreverGastosMesAsync(int mes, int ano)
    {
        // Calcula m�dia dos �ltimos 3 meses para cada categoria
 ...
 for (int i = 1; i <= 3; i++)
  ...
  ValorPrevisto = gastos.Average(),
   BaseadoEm = $"M�dia dos �ltimos 3 meses"
     });
            }
        }
...
public class ComparacaoMensal
{
    ...
    public string StatusReceitas => DiferencaReceitas >= 0 ? "?? Aumento" : "?? Redu��o";
    public string StatusDespesas => DiferencaDespesas >= 0 ? "?? Aumento" : "?? Redu��o";
}
