...
/// <summary>
/// Servi�o para importa��o de extratos banc�rios em formato CSV
/// </summary>
public class ImportacaoExtratoService
{
  ...
  public ImportacaoExtratoService(DatabaseService databaseService)
    ...
    /// <summary>
    /// Importa transa��es de um arquivo CSV
/// </summary>
     ...
     FormatoCSV formato = FormatoCSV.Padrao)
{
   ...
   {
       mensagens.Add("Arquivo n�o encontrado.");
...
var linhas = await File.ReadAllLinesAsync(caminhoArquivo);
     ...
     foreach (var linha in linhasDados)
 {
   try
  {
   ...
   mensagens.Add($"Erro na linha: {ex.Message}");
      ...
      mensagens.Add($"Importa��o conclu�da: {sucesso} transa��es importadas, {erros} erros.");
        }
 catch (Exception ex)
    {
 ...
 {
        // Formato: Data;Descri��o;Valor;Tipo
...
Tipo = campos[3].Trim().ToLower() == "receita" ? "Receita" : "Despesa",
       Efetivada = true,
    CategoriaId = 1 // Categoria padr�o - deve ser categorizado depois
        };
    }

    private Transacao? ParseNubank(string linha, int contaId)
    {
  // Formato Nubank: Data,Categoria,T�tulo,Valor
   var campos = linha.Split(',');
 ...
 return new Transacao
  ...
  {
  // Formato Inter: Data;Descri��o;Valor;Saldo
        var campos = linha.Split(';');
        ...
        return new Transacao
{
  ...
  {
        // Formato Ita�: data;lan�amento;ag.origem;valor;saldo
   var campos = linha.Split(';');
 ...
 Valor = Math.Abs(valor),
  ...
  private Transacao? ParseBradesco(string linha, int contaId)
    {
  // Formato Bradesco: Data;Hist�rico;Docto.;Cr�dito;D�bito;Saldo
   var campos = linha.Split(';');
...
{
      { "Alimenta��o", new List<string> { "restaurante", "mercado", "supermercado", "padaria", "lanche", "ifood", "uber eats" } },
 { "Transporte", new List<string> { "uber", "99", "combust�vel", "gasolina", "estacionamento", "ped�gio" } },
    { "Sa�de", new List<string> { "farm�cia", "droga", "hospital", "m�dico", "cl�nica", "laborat�rio" } },
  { "Lazer", new List<string> { "cinema", "teatro", "show", "netflix", "spotify", "amazon prime" } },
 { "Moradia", new List<string> { "aluguel", "condom�nio", "iptu", "luz", "�gua", "g�s", "internet" } }
 };
