...
/// <summary>
/// Servi�o de acesso aos dados usando SQLite
/// </summary>
...
await AtualizarSaldoContaAsync(transacao.ContaId);
        ...
        if (transacao.Tipo == "Transfer�ncia" && transacao.ContaDestinoId.HasValue)
        {
...
await AtualizarSaldoContaAsync(transacao.ContaId);
 ...
 .Where(t => t.ContaId == contaId && t.Efetivada)
     ...
     saldo -= transacao.Valor;
            else if (transacao.Tipo == "Transfer�ncia")
            {
if (transacao.ContaId == contaId)
  ...
  conta.DataAtualizacao = DateTime.Now;
    ...
    public async Task<List<OrcamentoMensal>> ObterOrcamentosMesAsync(int mes, int ano)
    {
      await InicializarAsync();
        return await _database!.Table<OrcamentoMensal>()
          .Where(o => o.Mes == mes && o.Ano == ano && o.Ativo)
     .ToListAsync();
    }

    public async Task<OrcamentoMensal?> ObterOrcamentoPorCategoriaAsync(int categoriaId, int mes, int ano)
    {
        await InicializarAsync();
        return await _database!.Table<OrcamentoMensal>()
   ...
   o.Ativo)
    ...
    public async Task<int> SalvarOrcamentoAsync(OrcamentoMensal orcamento, bool preserveSyncMetadata = false)
    {
     await InicializarAsync();
 ...
 foreach (var orcamento in orcamentos)
   ...
   t.Data <= dataFim)
    ...
    public async Task<int> ExcluirOrcamentoAsync(OrcamentoMensal orcamento)
    {
        await InicializarAsync();
        // Soft delete
 ...
 .OrderBy(m => m.Concluida)
  ...
  return await _database!.Table<Meta>()
    ...
    public async Task<int> SalvarMetaAsync(Meta meta, bool preserveSyncMetadata = false)
    {
     await InicializarAsync();
