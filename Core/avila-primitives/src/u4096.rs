...
use avila_nucleus::bits::u4096_ops::*;
...
impl Add for U4096 {
    ...
    fn add(self, rhs: Self) -> Self {
        let (result, _) = add4096(&self.0, &rhs.0);
        Self(result)
    }
}
...
impl Sub for U4096 {
    ...
    fn sub(self, rhs: Self) -> Self {
        let (result, _) = sub4096(&self.0, &rhs.0);
        Self(result)
    }
}
...
impl Mul for U4096 {
    ...
    fn mul(self, rhs: Self) -> Self {
        let result = mul4096x4096(&self.0, &rhs.0);
        let mut limbs = [0u64; 64];
        limbs.copy_from_slice(&result[0..64]);
        Self(limbs)
    }
}
...
impl Div for U4096 {
    ...
    fn div(self, rhs: Self) -> Self {
        let (quotient, _) = div4096(&self.0, &rhs.0);
        Self(quotient)
    }
}
...
impl Rem for U4096 {
    ...
    fn rem(self, rhs: Self) -> Self {
        let (_, remainder) = div4096(&self.0, &rhs.0);
        Self(remainder)
    }
}

// Bitwise operations
impl BitAnd for U4096 {
    type Output = Self;
...
// Shift operations
impl Shl<u32> for U4096 {
    type Output = Self;

    fn shl(self, rhs: u32) -> Self {
        Self(shl4096(&self.0, rhs))
    }
}

impl Shr<u32> for U4096 {
    type Output = Self;

    fn shr(self, rhs: u32) -> Self {
        Self(shr4096(&self.0, rhs))
    }
}
...
impl Ord for U4096 {
    fn cmp(&self, other: &Self) -> Ordering {
        if lt4096(&self.0, &other.0) {
            Ordering::Less
        } else if eq4096(&self.0, &other.0) {
            Ordering::Equal
        } else {
            Ordering::Greater
        }
...
// Display
impl core::fmt::Debug for U4096 {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "U4096([{:016x}", self.0[63])?;
        for i in (0..63).rev() {
...
impl core::fmt::Display for U4096 {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "U4096({})", self.0[0])
    }
}
...
mod tests {
    ...
    fn test_basic_arithmetic() {
        let a = U4096::from_u64(100);
        let b = U4096::from_u64(50);
    ...
    #[test]
    fn test_division() {
        let a = U4096::from_u64(107);
        let b = U4096::from_u64(10);
